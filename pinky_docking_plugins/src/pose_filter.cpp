...
geometry_msgs::msg::PoseStamped PoseFilter::update(
  const geometry_msgs::msg::PoseStamped & measurement)
{
  ...
  if ((rclcpp::Time(measurement.header.stamp) - pose_.header.stamp).seconds() > timeout_) {
    ...
    tf2::Quaternion filtered_quat, measurement_quat;
    tf2::fromMsg(measurement.pose.orientation, measurement_quat);
    tf2::fromMsg(pose_.pose.orientation, filtered_quat);
    filtered_quat = filtered_quat.slerp(measurement_quat, coef_);
    pose_.pose.orientation = tf2::toMsg(filtered_quat);
  }
...
{
  filt = (1.0 - coef_) * filt + coef_ * meas;
}
